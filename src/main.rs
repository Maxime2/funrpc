...
fn main() {
    ...
    match opt.cmd {
        ...
        Command::RawTx { doc } => {
            ...
            if opt.verbose > 0 {
                println!("Sending raw signed transaction to {}:{}", opt.hostname, opt.port);
            }
        }
    }
...
actix::run(
    ...
    client::ClientRequest::get(url) // <- Create request builder
            .header("User-Agent", "Actix-web")
            .body(actix_web::Body::from(body)).unwrap()
            .send()
            .map_err(|_| ())
